$(function(){
	...
	function selectList(pageNum){
		...
		success:function(param){
			...
			$(param.list).each(function(index,item){
				...
				if(item.re_date){
					output += '<span class="modify-date">최근 수정일 : '+ item.re_date + '</span>';
				}else{
					output += '<span class="modify-date">등록일 : '+ item.re_mdate + '</span>';
				}
				output += '</li>';
				output += '</ul>';
	...
	$(document).on('click','.modify-btn',function(){
		...
		//이전에 이미 수정하는 댓글이 있을 경우 수정버튼을 클락하면 숨김
		//sub-item을 환원시키고 수정 폼을 초기화함
		initModifyForm();
		//지금 버튼 클릭해서 수정하고자 하는 데이터는 감추기
